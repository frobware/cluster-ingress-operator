...
// its value is "false").
func HTTP2IsDisabled(ci *operatorv1.IngressController, ingressConfig *configv1.Ingress) bool {
	controllerHasHTTP2Annotation, controllerHasHTTP2Disabled := HTTP2IsDisabledByAnnotation(ci.Annotations)
	_, configHasHTTP2Disabled := HTTP2IsDisabledByAnnotation(ingressConfig.Annotations)
	if controllerHasHTTP2Disabled {
		return true
	}
	if configHasHTTP2Disabled {
