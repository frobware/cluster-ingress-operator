...
// isolateIngressControllerToNamespace configures an IngressController
// to process only routes and resources from a specific namespace,
// ensuring test isolation.
//
// This function achieves isolation by:
//
//  1. Labelling the provided namespace (`ns`) with shard-specific
//     labels derived from the namespace and IngressController names.
//     These labels act as the association between the namespace and
//     the IngressController. The label key is derived as
//     "<namespace-name>-shard" and the value is set to the
//     IngressController's name.
//
//  2. Configuring the provided IngressController (`ic`) with a
//     NamespaceSelector that matches the shard-specific labels. This
//     restricts the IngressController to process only namespaces that
//     have these labels.
//
// This function ensures:
//
//   - The router managed by the IngressController reloads only for
//     configuration changes related to the IngressController itself
//     or the labelled namespace.
//
//   - Other namespaces and unrelated configuration changes do not
//     trigger router reloads, maintaining strict isolation for the
//     test.
//
// Example:
//
// For a namespace with `ns.Name = "test-namespace"` and an
// IngressController with `ic.Name = "test-ingress"`, the shard label
// applied to the namespace and used in the NamespaceSelector will be:
// "test-namespace-shard": "test-ingress".
func isolateIngressControllerToNamespace(t *testing.T, ns *corev1.Namespace, ic *operatorv1.IngressController) error {
	labels := map[string]string{ns.Name + "-shard": ic.Name}

	if err := applyResourceLabels(context.Background(), kclient, ns, labels); err != nil {
		return fmt.Errorf("failed to label namespace %q with shard labels %v: %w", ns.Name, labels, err)
	}
	...
	if err := updateIngressControllerWithRetryOnConflict(t, icName, 30*time.Second, func(ic *operatorv1.IngressController) {
		ic.Spec.NamespaceSelector = &metav1.LabelSelector{
			MatchLabels: labels,
		}
	}); err != nil {
		return fmt.Errorf("failed to update NamespaceSelector for IngressController %q in namespace %q with shard labels %v: %w", ic.Name, ic.Namespace, labels, err)
	}
...
// - Ensures the object has a labels map, creating one if necessary.
// - Retains any existing labels not specified in the input map.
// - Retries the operation if a conflict occurs due to concurrent updates.
//
// Limitations:
// - The object (`obj`) must have a valid namespace and name set.
// - Only works for objects that implement the client.Object interface.
func applyResourceLabels(ctx context.Context, kclient client.Client, obj client.Object, labels map[string]string) error {
...
func Test_IdleConnectionTerminationPolicy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(string(tc.policy), func(t *testing.T) {
			...
			if err := isolateIngressControllerToNamespace(t, ns, ic); err != nil {
